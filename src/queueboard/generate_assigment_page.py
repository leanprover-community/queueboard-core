...
def compute_pr_list_from_aggregate_data_only(
    aggregate_data: dict[int, AggregatePRInfo],
    ...
    prs_not_from_fork = [pr for pr in nondraft_PRs if aggregate_data[pr.number].head_repo == "leanprover-community"]
    return determine_pr_dashboards(all_open_prs, nondraft_PRs, base_branch, prs_not_from_fork, CI_status, aggregate_data, True)
...
def main() -> None:
    ...
    thead = _write_table_header(
        ["User", open_assigned, "Number", f'<a title="{explanation}">Weighted number</a>', all_recent, ""], "    "
    )
    tbody = ""
    for name, (prs, n_weighted, n_all) in stats.assignments.items():
        ...
        tbody += _write_table_row(
            [user_link(name), ", ".join(formatted_prs), str(len(prs)), f"{n_weighted:.1f}", str(n_all), ""], "    "
        )
    table = f"  <table>\n{thead}{tbody}  </table>"
    ...
    suggestions = {n: (val.code, val.all_potential_reviewers) for (n, val) in suggestions_pre.items()}
    ...
    table = write_dashboard(
        "assign-reviewer.html", pr_lists, Dashboard.Queue, parsed, settings, False, suggestions, "propose-reviewers-all"
    )
    propose_all = f"{header}\n{table}\n"
    write_webpage(
        f"{title}\n{welcome}\n{update}\n{stats_section}\n{reviewers}\n{propose_all}\n{propose_stale}",
        "assign-reviewer.html",
        custom_script=tables_configuration_script(ALIAS_MAPPING, ""),
    )

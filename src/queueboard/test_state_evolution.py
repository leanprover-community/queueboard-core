...
from queueboard.state_evolution import (
    PRState,
    PRStatus,
    Event,
    Metadata,
    total_queue_time_inner,
    determine_state_changes,
    first_on_queue_inner,
    last_status_update_inner,
)
...
def test_determine_state_changes() -> None:
    ...
    check(
        [Event.update_ci_status(dummy, CIStatus.Pass), Event.update_ci_status(dummy, CIStatus.Fail)],
        PRState.with_labels_and_ci([], CIStatus.Fail),
    )
    check(
        [
            Event.update_ci_status(dummy, CIStatus.Pass),
            Event.draft(dummy),
            Event.update_ci_status(dummy, CIStatus.Running),
            Event.undraft(dummy),
        ],
        PRState.with_labels_and_ci([], CIStatus.Running),
    )
    ...
    check(
        [Event.add_label(dummy, "t-data"), Event.add_label(dummy, "WIP"), Event.remove_label(dummy, "t-data")],
        PRState.with_labels([LabelKind.WIP]),
    )
    # Adding two labels.
    check([Event.add_label(dummy, "awaiting-author")], PRState.with_labels([LabelKind.Author]))
    check(
        [Event.add_label(dummy, "awaiting-author"), Event.add_label(dummy, "WIP")],
        PRState.with_labels([LabelKind.Author, LabelKind.WIP]),
    )
    check([Event.add_label(dummy, "awaiting-author"), Event.remove_label(dummy, "awaiting-author")], PRState.with_labels([]))
    check(
        [
            Event.add_label(dummy, "awaiting-author"),
            Event.remove_label(dummy, "awaiting-author"),
            Event.add_label(dummy, "awaiting-zulip"),
        ],
...
def test_total_queue_time() -> None:
    ...
    check_basic(
        sep(1),
        sep(10),
        [Event.add_label(sep(1), "blocked-by-other-PR"), Event.add_label(sep(6), "merge-conflict")],
    ...
    check_basic(
        sep(1),
        sep(10),
        [Event.add_label(sep(1), "blocked-by-other-PR"), Event.remove_label(sep(6), "blocked-by-other-PR")],
    ...
    check_basic(
        ...
        sep(10),
        [
            Event.add_label(sep(1), "blocked-by-other-PR"),
            Event.remove_label(sep(6), "blocked-by-other-PR"),
            Event.add_label(sep(8), "WIP"),
        ],
    ...
    check_basic(
        ...
        [
            Event.add_label(sep(1), "blocked-by-other-PR"),
            Event.remove_label(sep(8), "blocked-by-other-PR"),
            Event.add_label(sep(10), "WIP"),
        ],
        relativedelta(days=2),
    )
    check_basic(
        sep(1),
        sep(10),
        [Event.add_label(sep(1), "blocked-by-other-PR"), Event.remove_label(sep(8), "blocked-by-other-PR")],
        relativedelta(days=2),
    )
    ...
    check_basic(
        sep(1),
        sep(10),
        [Event.add_label(sep(1), "CI"), Event.remove_label(sep(3), "CI"), Event.add_label(sep(8), "WIP")],
        relativedelta(days=7),
    )

    # A PR getting blocked.
    check_basic(
        sep(1), sep(10), [Event.add_label(sep(1), "blocked-by-other-PR"), Event.add_label(sep(8), "easy")], relativedelta(days=0)
    )
    ...
    check_basic(
        ...
        [
            Event.add_label(sep(1), "blocked-by-other-PR"),
            Event.add_label(sep(3), "new-contributor"),
            Event.add_label(sep(12), "WIP"),
            Event.add_label(sep(14), "AIMS"),
            Event.remove_label(sep(16), "new-contributor"),
            Event.remove_label(sep(18), "blocked-by-other-PR"),
        ],
        relativedelta(days=0),
    )
    ...
    check_basic(
        ...
        [
            ...
            Event.add_label(sep(13), "new-contributor"),
            Event.add_label(sep(14), "WIP"),
            Event.add_label(sep(16), "AIMS"),
            Event.remove_label(sep(16), "new-contributor"),
            Event.remove_label(sep(18), "blocked-by-other-PR"),
        ],
    ...
    events = [
        ...
        Event.add_label(july(1), "awaiting-author"),
        Event.remove_label(july(1), "awaiting-author"),
        Event.add_label(july(1), "awaiting-review-DONT-USE"),
        Event.remove_label(july(1), "awaiting-review-DONT-USE"),
        Event.add_label(july(1), "awaiting-author"),
        Event.remove_label(july(2), "awaiting-author"),
        Event.add_label(july(2), "WIP"),
        Event.remove_label(july(13), "WIP"),
        # is now without essential labels -> reviewable
        Event.add_label(july(13), "help-wanted"),
        Event.add_label(aug(8), "awaiting-author"),  # waiting for help
        Event.add_label(aug(15), "t-number-theory"),
        Event.remove_label(sep(3), "awaiting-author"),  # help-wanted
    ...
    events = [
        Event.draft(sep(3)),
        Event.add_label(sep(10), "t-meta"),
        Event.undraft(sep(10)),
        Event.draft(sep(15)),
...
def test_last_status_update():
    def check_with_initial(now: datetime, state: Metadata, expected: Tuple[datetime, relativedelta, PRStatus]) -> None:
        ...
        assert absolute == expected[0], (
            f"basic test failed: expected last update on {expected[0]} in review, obtained {absolute} instead"
        )
        assert relative == expected[1], (
            f"basic test failed: expected last update on {expected[1]} in review, obtained {relative} instead"
        )
    ...
    events = [
        ...
        Event.add_label(sep(16), "AIMS"),
        Event.remove_label(sep(16), "new-contributor"),
        Event.remove_label(sep(18), "AIMS"),
    ]
    ...
    events = [
        ...
        Event.add_label(sep(16), "AIMS"),
        Event.remove_label(sep(16), "new-contributor"),
        Event.remove_label(sep(19), "blocked-by-other-PR"),
    ]
    ...
    events = [
        ...
        Event.add_label(aug(8), "awaiting-author"),
        Event.add_label(aug(15), "t-number-theory"),
        Event.remove_label(sep(3), "awaiting-author"),
        Event.remove_label(sep(20), "help-wanted"),
        Event.add_label(sep(22), "awaiting-author"),
        Event.remove_label(sep(23), "awaiting-author"),
    ]

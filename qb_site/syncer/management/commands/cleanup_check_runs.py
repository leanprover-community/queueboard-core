...
class Command(BaseCommand):
    help = "Delete outdated CheckRun snapshot rows, keeping the latest per (head_sha, name)."
    ...
    def _latest_snapshot_ids_for_pr(self, pr_id: int) -> list[int]:
        qs = CheckRun.objects.filter(pull_request_id=pr_id, github_node_id__isnull=False)
        return list(
            qs.annotate(ts=Coalesce("gh_completed_at", "gh_started_at"))
            .order_by("head_sha", "name", "-ts", "-id")
    ...
    def handle(self, *args, **options) -> None:
        ...
        for pr in self._iter_prs(repo=repo, include_closed=include_closed, limit=limit, offset=offset):
            ...
            stale_qs = CheckRun.objects.filter(

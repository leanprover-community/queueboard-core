...
class QueueboardSnapshotView(APIView):
    """Serve cached queueboard snapshots with simple cache validation."""
...
def _etag_matches(raw: str | None, etag: str) -> bool:
    ...
    weak_forms = {f'W/"{etag}"', f"W/{etag}"}
    return etag in candidates or f'"{etag}"' in candidates or any(tag in candidates for tag in weak_forms)


def _is_not_modified(request, snapshot: QueueSnapshot) -> bool:

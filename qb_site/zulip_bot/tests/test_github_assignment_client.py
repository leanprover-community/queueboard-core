...
from zulip_bot.services.assignment_execution import AssignmentMutationError, GitHubAssignmentClient
...
class TestGitHubAssignmentClient(SimpleTestCase):
    ...
    def test_assign_calls_issues_assignees_endpoint(self) -> None:
        with patch(
            "zulip_bot.services.assignment_execution.requests.request", return_value=self._response(status_code=200)
        ) as mock_request:
    ...
    def test_unassign_maps_422_to_validation_failed(self) -> None:
        with patch(
            "zulip_bot.services.assignment_execution.requests.request",
    ...
    def test_assign_maps_403_to_permission_denied(self) -> None:
        with patch("zulip_bot.services.assignment_execution.requests.request", return_value=self._response(status_code=403)):
            client = GitHubAssignmentClient(token="tok")
            with self.assertRaises(AssignmentMutationError) as exc:
                client.assign(owner="o", repo="r", number=3, github_login="alice")

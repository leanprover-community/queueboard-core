...
- Two spellings of one command in the same policy collapse into a single rule, with the last one
  winning. `manage.py zulip_policy validate` refuses that outright; the runtime loader, which cannot
  refuse its own config, logs `zulip_command_policy_duplicate_entry`. `zulip_policy sync` rewrites
  keys in canonical form (rules preserved), so it never appends a second spelling of a command that
  is already present.

...
- CheckRun node ids are not always stable across state transitions; we defensively merge by `(head_sha, name)` when interpreting pending CI and prune older snapshot rows per `(head_sha, name)` so stale queued rows do not block progression. A future refactor could additionally fall back to `external_id`.

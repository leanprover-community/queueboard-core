...
The queueboard is outgrowing the current GitHub Actions setup (cf. `docs/queueboard_main_workflow.md`) and we now have some resources from the Mathlib Initiative, so I spent some time thinking about what a self-hosted version 2 might look like, inspired by the principles of the [12 factor app](https://12factor.net/).
...
For a detailed map of the legacy pipeline’s data surface and flow, see docs/legacy_data_surface.md. For how v2 ingests GitHub data efficiently using a single per‑PR GraphQL bundle and persists raw facts, see docs/syncer_ingestion_plan.md.
...
See docs/syncer_ingestion_plan.md for the concrete ingestion architecture, bundle query shape, and services.
...
Note: In v2 this lives in the raw/syncer schema (label definitions plus PR↔label attachments), not in `core`.

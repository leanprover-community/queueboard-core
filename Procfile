...
worker: sh -c 'export PYTHONPATH=$PWD/qb_site:$PWD${PYTHONPATH:+:$PYTHONPATH}; celery -A qb_site beat -l info --schedule /tmp/celerybeat-schedule & celery -A qb_site worker -l info -Q default --concurrency=2 --prefetch-multiplier=1 --max-tasks-per-child=25 --max-memory-per-child=200000 --without-gossip --without-mingle'
